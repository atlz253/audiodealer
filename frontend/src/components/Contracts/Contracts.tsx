  ...
  return (
    <Table hover>
      <thead className={styles.head}>
        <tr>
          <th onClick={sortByContractID} style={{ cursor: "pointer" }}>
            Номер договора
          </th>
          <th>Продавец</th>
          <th>Покупатель</th>
          <th onClick={sortByContractType} style={{ cursor: "pointer" }}>
            Тип договора
          </th>
          <th onClick={sortByStatusType} style={{ cursor: "pointer" }}>
            Статус
          </th>
          <th>Сумма</th>
          <th>Дата создания</th>
      ...
      <tbody className={styles.body}>
        {contracts.map((contract: IBaseContract) => (
          ...
          >
            <td cell-name="Номер договора" className={styles.cell}>
              {contract.id}
            </td>
            <td cell-name="Продавец" className={styles.cell}>
              {contract.sellerName}
            </td>
            <td cell-name="Покупатель" className={styles.cell}>
              {contract.buyerName}
            </td>
            <td cell-name="Тип договора" className={styles.cell}>
              {contract.type}
            </td>
            <td cell-name="Статус" className={styles.cell}>
              {contract.status}
            </td>
            <td cell-name="Сумма" className={styles.cell}>
              {contract.price}
            </td>
            <td cell-name="Дата создания" className={styles.cell}>
              {contract.created}
            </td>
          </tr>
        ))}
