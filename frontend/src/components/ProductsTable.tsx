...
const ProductsTable: FC<IProductTableProps> = ({
  ...
  const isProductsHaveDeliveryDays =
    products.length > 0 && products[0].deliveryDays !== undefined;
  const isProductsHaveDeliveryDate =
    products.length > 0 && products[0].deliveryDate !== undefined;

  const onDeliveryDateChange = (productID: number, value: string) => {
    if (setProducts === undefined) {
      console.warn(
        "Невозможно поменять значение т к не была передана функция изменения состояния",
      );
            ...
            {isProductsHaveDeliveryDays && (
              <td>
                <Form.Control
                  ...
                  onChange={(event) =>
                    onDeliveryDateChange(product.id, event.target.value)
                  }
                  disabled={
                    setProducts === undefined ||
                    addedProducts.filter((p) => p.id === product.id).length !==
                      0
                  } // TODO: Изменение времени доставки после окончания редактирования поля
                />
              </td>
            )}
