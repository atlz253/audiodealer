...
usersRouter.get(
  "/",
  expressAsyncHandler(
    async (req: RequestBody, res: Response<IUser[]>, next: NextFunction) => {
...
usersRouter.post(
  "/new",
  expressAsyncHandler(
    async (req: RequestBody<IUser>, res: Response<ID>, next: NextFunction) => {
      ...
      switch (req.body.type) {
        ...
        default:
          throw new Error(
            "Не удалось определить тип добавляемого пользователя",
          );
      }
...
usersRouter.put(
  "/:userID",
  expressAsyncHandler(
    async (req: RequestBody<IUser>, res: Response, next: NextFunction) => {
      switch (req.body.type) {
        case "admin":
...
usersRouter.delete(
  ...
  expressAsyncHandler(
    async (req: RequestBody, res: Response, next: NextFunction) => {
      ...
      Logger.info(
        `${req.jwt?.login} удалил пользователя с ID ${req.params.userID}`,
      );
